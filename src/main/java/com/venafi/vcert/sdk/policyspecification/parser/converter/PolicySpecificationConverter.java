...
public abstract class PolicySpecificationConverter<T> {
    ...
    public abstract PolicySpecification convert(T t) throws Exception;

    enum TypePSAttToCreate {
        NORMAL, DEFAULT, NONE;
    }

    protected <S> TypePSAttToCreate shouldCreateAttribute(S source, Function<S, AttributeLockable> sourceGetter){
        ...
        if (attributeLockable != null && attributeLockable.values().length > 0 ) {
            if (attributeLockable.lock() )
                return TPPPolicyToPolicySpecificationConverter.TypePSAttToCreate.NORMAL;
            else
                return TPPPolicyToPolicySpecificationConverter.TypePSAttToCreate.DEFAULT;
        }

        return TPPPolicyToPolicySpecificationConverter.TypePSAttToCreate.NONE;
    }
