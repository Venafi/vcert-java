...
import com.venafi.vcert.sdk.connectors.cloud.domain.Application.OwnerIdsAndType;
...
public class CloudConnectorUtils {
	...
	public static void setCitToApp(String policyName, CertificateIssuingTemplate cit, String[] usersList, String apiKey,
		...
		// then it will need to create it
		if( application == null )
	...
	private static void createAppForCit(CertificateIssuingTemplate cit, String appName, String[] userList, String apiKey,
			Cloud cloud) throws VCertException {
		Application application = new Application();

		// Obtaining the owners from the user list
		List<OwnerIdsAndType> ownersList = CloudConnectorUtils.resolveOwners(userList, apiKey, cloud);
	...
	private static void addCitToApp(CertificateIssuingTemplate cit, Application application,
			String[] usersList, String apiKey, Cloud cloud) throws VCertException {
		Map<String, String> citAliasIdMap;
		...
		if ( !citAliasIdMap.containsKey(cit.name()) ) {
			...
			List<OwnerIdsAndType> ownersList =  CloudConnectorUtils.resolveOwners(usersList, apiKey, cloud);
			application.ownerIdsAndTypes(ownersList);
	...
	private static List<OwnerIdsAndType> resolveOwners(String[] usersList, String apiKey, Cloud cloud) {
	...
	private static String readZipEntry(ZipEntry zipEntry, ZipInputStream zis, String certId) throws VCertException, IOException {
		...
		final int MAX_RATIO = 3;//It's expected the compression ratio to not be more than 3
